...
\title{MAC5921  - Deep Learning} % Title of the assignment
...
\section*{Convolutional Neural Networks} % Unnumbered section


Convolutional Neural Networks (CNNs) are suitable for tasks like pattern recognition, object detection and segmentation in images. Their core component are the convolional layers that perform the convolution operation. Such operation have some details worth highlighting as they let us understand and hypothesize the success that CNNs have in tasks that require image processing.


\begin{itemize}
	\item \textbf{Stride:} this parameter tells the "pixel speed" that the convolution filter (kernel) will parse the target image. For example, for a unary stride, the kernel moves to the next position one pixel at a time. For stride equal 2, it does so at 2 pixel at a time and so on. The greater the stride, the more \textit{localized} will be the features extracted via the kernel.
	\item \textbf{Padding:} this parameter tells how many extra pixels will be added to the image borders so that the kernel can operate over the pixels close and exactly on those borders. The greater this parameter, the greater will be the dimensions of the output as more pixels in the input means more operations with the convolution filter.
